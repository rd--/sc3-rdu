...
/* The input layout has the buffer index at zero the input signal at
   ...
   seconds.  */
void DelayMap_Setup(DelayMap *unit)
{
	int i, j;
...
float DelayMap_step(DelayMap *unit, float s)
{
	float v_out = 0.0;
	for (int i = 0; i < unit->m_map_n; i++) {
		...
		if (unit->m_map[i].src < 0) {
			...
			v_src = (unit->m_map[i].gain * cq_access_i(unit->m_signal, unit->m_signal_n, unit->m_map[i].src, unit->m_write_index));
		}
...
void DelayMap_next(DelayMap *unit, int inNumSamples)
{
	rdu_get_buf(dl, 0);
	rdu_check_buf(dl, 1);
	if ((int)IN0(2) > 0) {
	...
	for (int i = 0; i < inNumSamples; i++) {
		out[i] = DelayMap_step(unit, in[i]);
	}
}

void DelayMap_Ctor(DelayMap *unit)
{
	rdu_init_buf(dl);
	unit->m_map_n = (unit->mNumInputs - Map_Offset) / 4;
	DelayMap_Setup(unit);
	SETCALC(DelayMap_next);
	unit->m_write_index = 0;
	unit->m_signal = NULL;
	DelayMap_next(unit, 1);
}
